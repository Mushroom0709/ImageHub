...
def list_review_assets(
    ...
    if pending_tag_ids:
        # 有 pending 标签的素材
        subq = db.query(AssetTag.asset_id).filter(
            AssetTag.tag_id.in_(pending_tag_ids)
        ).distinct()
        query = query.filter(Asset.id.in_(subq))
    else:
        # 没有 pending 标签，检查低置信度
        subq = db.query(AssetTag.asset_id).filter(
            AssetTag.confidence < 0.7
        ).distinct()
        query = query.filter(Asset.id.in_(subq))
    ...
    for asset in assets:
        ...
        for tag, confidence, source in tags:
            needs_review = tag.status == "pending" or (confidence is not None and confidence < 0.7)
            review_tags.append({
                "id": str(tag.id),
                "name": tag.name,
